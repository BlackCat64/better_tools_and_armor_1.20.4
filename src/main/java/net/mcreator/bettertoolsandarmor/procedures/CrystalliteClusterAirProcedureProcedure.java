				...
				if (found == true && y < 60) {
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(found_x, found_y, found_z), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("better_tools:crystallite_shimmer")), SoundSource.BLOCKS, 10, 1);
						} else {
							_level.playLocalSound(found_x, found_y, found_z, BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("better_tools:crystallite_shimmer")), SoundSource.BLOCKS, 10, 1, false);
						}
					}
				}
			}
