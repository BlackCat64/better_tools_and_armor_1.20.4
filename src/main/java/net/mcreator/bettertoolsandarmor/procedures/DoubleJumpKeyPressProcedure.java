		...
		if (!(new Object() {
			...
			if (!entity.onGround() && entity.getDeltaMovement().y() < 0.5) {
				if (!entity.isSwimming()) {
					if (entity instanceof LivingEntity _livEnt4 && _livEnt4.hasEffect(BetterToolsModMobEffects.DOUBLE_JUMP.get()) && entity.getData(BetterToolsModVariables.PLAYER_VARIABLES).extra_jumps > 0) {
						if (entity.getData(BetterToolsModVariables.PLAYER_VARIABLES).time_since_last_jumped >= 4) {
