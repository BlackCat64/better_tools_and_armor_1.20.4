...
public class GlassArmorProcedureProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
			...
			if ((entity instanceof LivingEntity _livEnt && _livEnt.hasEffect(MobEffects.INVISIBILITY) ? _livEnt.getEffect(MobEffects.INVISIBILITY).getDuration() : 0) <= 1) {
				if (!((entity instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.FEET) : ItemStack.EMPTY)
					...
					if (!(entity instanceof ServerPlayer _plr51 && _plr51.level() instanceof ServerLevel
							&& _plr51.getAdvancements().getOrStartProgress(_plr51.server.getAdvancements().get(new ResourceLocation("better_tools:glass_armor_adv"))).isDone())) {
						if (entity instanceof ServerPlayer _player) {
							AdvancementHolder _adv = _player.server.getAdvancements().get(new ResourceLocation("better_tools:glass_armor_adv"));
							if (_adv != null) {
		...
		if (entity instanceof LivingEntity && ((LivingEntity) entity).getAttribute(NeoForgeMod.NAMETAG_DISTANCE.value()) != null) {
			default_nametag_range = ((LivingEntity) entity).getAttribute(NeoForgeMod.NAMETAG_DISTANCE.value()).getValue();
			if (armor_active) {
				{
					Entity _ent = entity;
