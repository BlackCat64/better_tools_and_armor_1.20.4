	...
	public static void execute(Entity entity, ItemStack itemstack) {
		...
		if ((entity instanceof LivingEntity _livEnt && _livEnt.hasEffect(MobEffects.NIGHT_VISION) ? _livEnt.getEffect(MobEffects.NIGHT_VISION).getDuration() : 0) < 205) {
			...
			if (!(itemstack.getDamageValue() >= itemstack.getMaxDamage() - cost)) {
				...
				if (!(entity instanceof ServerPlayer _plr15 && _plr15.level() instanceof ServerLevel && _plr15.getAdvancements().getOrStartProgress(_plr15.server.getAdvancements().get(new ResourceLocation("better_tools:night_phantom_adv"))).isDone())
						&& entity instanceof LivingEntity _livEnt16 && _livEnt16.hasEffect(MobEffects.SLOW_FALLING)) {
					if ((entity instanceof LivingEntity _entGetArmor ? _entGetArmor.getItemBySlot(EquipmentSlot.FEET) : ItemStack.EMPTY).getItem() == BetterToolsModItems.PHANTOM_BOOTS.get()
						...
						if (entity instanceof ServerPlayer _player) {
							AdvancementHolder _adv = _player.server.getAdvancements().get(new ResourceLocation("better_tools:night_phantom_adv"));
							if (_adv != null) {
