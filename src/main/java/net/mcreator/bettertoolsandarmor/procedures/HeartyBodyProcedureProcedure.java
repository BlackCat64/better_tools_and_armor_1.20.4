	...
	public static void execute(Entity entity, ItemStack itemstack) {
		...
		if (!(entity instanceof LivingEntity _livEnt0 && _livEnt0.hasEffect(MobEffects.ABSORPTION))) {
			...
			if (!(itemstack.getDamageValue() >= itemstack.getMaxDamage() - cost)) {
				...
				if (!(entity instanceof ServerPlayer _plr15 && _plr15.level() instanceof ServerLevel && _plr15.getAdvancements().getOrStartProgress(_plr15.server.getAdvancements().get(new ResourceLocation("better_tools:hearty_adv"))).isDone())) {
					if (entity instanceof ServerPlayer _player) {
						AdvancementHolder _adv = _player.server.getAdvancements().get(new ResourceLocation("better_tools:hearty_adv"));
						if (_adv != null) {
